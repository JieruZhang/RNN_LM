...
class RNN(object):
    ...
    def predict(self, x):
        ...
        for t in range(len(x)):
            s[t] = sigmoid(self.V[:,x[t]] + np.dot(s[t-1], np.transpose(self.U)))
            y[t] = softmax(np.dot(s[t], np.transpose(self.W)))
        return y, s
    ...
    def acc_deltas(self, x, d, y, s):
        ...
        for t in reversed(range(len(x))):
            ...
            #deltaV
            delta_in = np.multiply(np.dot(delta_out, self.W), np.multiply(s[t], 1-s[t]))
    ...
    def acc_deltas_bptt(self, x, d, y, s, steps):
        ...
        for t in reversed(range(len(x))):
            ...
            for tau in range(1,steps+1):
                if tau <= t +1:
                    delta_in = np.multiply(np.dot(delta_in, self.U), np.multiply(s[t-tau], (1-s[t-tau])))
...
if __name__ == "__main__":
    ...
    if mode == "train-lm":
        ...
        np.save('./LM_model/rnn.U.npy', r.U)
        np.save('./LM_model/rnn.V.npy', r.V)
        np.save('./LM_model/rnn.W.npy', r.W)


        adjusted_loss = adjust_loss(run_loss, fraction_lost,q)

        print('the corss entropy loss is:{}'.format(run_loss))
    ...
    if mode == "train-np":
        ...
        ##########################
        r = RNN(vocab_size, hdim, vocab_size)
        run_loss = r.train_np(X_train, D_train, X_dev, D_dev,epochs=10, learning_rate = lr, back_steps = lookback)
        np.save('./bi_model/rnn.U.npy', r.U)
        np.save('./bi_model/rnn.V.npy', r.V)
        np.save('./bi_model/rnn.W.npy', r.W)
        adjusted_loss = adjust_loss(run_loss, fraction_lost,q)
    ...
    if mode == "predict-lm":
        ...
        # q = vocab.freq[vocab_size] / sum(vocab.freq[vocab_size:])
        # fraction_lost = fraq_loss(vocab, word_to_num, vocab_size)
        # mean_loss = r.compute_mean_loss(X_np_dev,D_np_dev)
        ...
        S_np_test = docs_to_indices(docs, word_to_num, 1, 0)
        X_np_test, D_np_test = seqs_to_lmnpXY(S_np_test)
        np_acc = r.compute_acc_lmnp(X_np_test, D_np_test)
        print('Number prediction accuracy:', np_acc)

        q = vocab.freq[vocab_size] / sum(vocab.freq[vocab_size:])
        fraction_lost = fraq_loss(vocab, word_to_num, vocab_size)
        mean_loss = r.compute_mean_loss(X_np_test,D_np_test)
        adjusted_loss = adjust_loss(mean_loss, fraction_lost,q)
        print('Mean loss:{}'.format(mean_loss))
        print("Unadjusted: %.03f" % np.exp(mean_loss))
        print("Adjusted for missing vocab: %.03f" % np.exp(adjusted_loss))
